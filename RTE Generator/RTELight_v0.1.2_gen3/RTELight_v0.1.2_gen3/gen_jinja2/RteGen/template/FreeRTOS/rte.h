...
{
    RTE_pRunnable run;          /**< \brief pointer to the runnable */
    EventBits_t event;        /**< \brief event mask which will activate the runnable */
} RTE_eventTable_t;
 ...
 * \param uint16_t tableSize : [IN] table size
 * \param EventGroupHandle_t ev : [IN] event mask to be processed
 * \return RC_SUCCESS if all ok, error code otherwise
 */
RC_t RTE_ProcessEventTable(RTE_eventTable_t const *const table, uint16_t tableSize, EventBits_t ev);
