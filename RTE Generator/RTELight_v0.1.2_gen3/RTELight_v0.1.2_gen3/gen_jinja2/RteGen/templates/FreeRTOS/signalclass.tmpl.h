...
struct {{name.upper()}}_cfg_s
{
	 ...
	 * Event onUpdate
	 */
	EventBits_t const evOnUpdate;


	/**
	 * Event onError
	 */
	EventBits_t const evOnError;
 ...
 *
 **/
inline RC_t RTE_{{name.upper()}}_init( {{name.upper()}}_t * const signal, const {{name.upper()}}_data_t * const value )
{
...
inline RC_t RTE_{{name.upper()}}_setStatus( {{name.upper()}}_t * const signal, const RTE_signalStatus_t status )
{
	...
	if (signal->status == RTE_SIGNALSTATUS_INVALID)
	{
		...
		{
			xEventGroupSetBits( event, signal->pSigCfg->evOnError);
		}
	}
...
{
	if (TRUE == signal->pSigCfg->resourceActive)
	{
		GetResource(signal->pSigCfg->resource);
	}
	*data = signal->value;
	if (TRUE == signal->pSigCfg->resourceActive)
	{
		ReleaseResource(signal->pSigCfg->resource);
	}
...
inline RC_t RTE_{{name.upper()}}_set({{name.upper()}}_t * const signal, const {{name.upper()}}_data_t value  )
{
	...
	{
		xEventGroupSetBits( event, signal->pSigCfg->evOnUpdate);
	}
